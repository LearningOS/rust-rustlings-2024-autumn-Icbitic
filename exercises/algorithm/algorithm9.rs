    ...
    T: Default + Clone + Copy,
{
    ...
    fn next(&mut self) -> Option<T> {
        ...
        let root = self.items[1].clone(); // Get the root item
        self.items[1] = self.items[self.count]; // Move the last item to the root
        self.count -= 1; // Decrease the count
        self.bubble_down(1); // Bubble down the root to restore the heap property
        Some(root)
    }
}
