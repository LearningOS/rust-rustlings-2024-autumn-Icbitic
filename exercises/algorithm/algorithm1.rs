...
impl<T> LinkedList<T> {
        ...
        T: PartialOrd + Clone,
    {
        ...
        // SAFETY: We are safely traversing the linked lists by checking the pointers.
        unsafe {
            ...
            while let Some(a) = a_ptr {
                merged_list.add((*a.as_ptr()).val.clone());  // Clone the value
                a_ptr = (*a.as_ptr()).next;
            }

            // Add remaining nodes from list_b
            while let Some(b) = b_ptr {
                merged_list.add((*b.as_ptr()).val.clone());  // Clone the value
                b_ptr = (*b.as_ptr()).next;
            }
        }
